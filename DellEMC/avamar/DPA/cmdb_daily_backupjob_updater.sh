#!/bin/bash -x
#==============================================================================
#  SCRIPT.........:  /app/opt/emc/dpa/scripts/cmdb_daily_backupjob_updater.sh
...
if [ "$?" = "0" ]; then
	...
	STATUS="ALERT";
	cat $LOGFILE | mailx -s "cmdb_daily_backupjob_updater.sh - $STATUS" $EMAIL;
fi
...
#pull csv
wget --no-check-certificate --user=administrator --password=administrator https://cvglpdpa01.td.afg:9002/dpa-api/scheduledreport/results/cmdb_daily_backupjob_updater.csv
error_exit_func

#make any modificatons
##exclude ==== egrep -v Default, Container, test, tst


#send to isilon
mv cmdb_daily_backupjob_updater.csv Avamar.txt
smbclient -U ga/cvgavmvic%0Bj5CIwlgdYa //cvgisln01.nas.afg/sdrive --directory Shared/Oncall/CMDB -c 'put "Avamar.txt"'
error_exit_func

#remove file
rm -f ./cmdb_daily_backupjob_updater*
error_exit_func
