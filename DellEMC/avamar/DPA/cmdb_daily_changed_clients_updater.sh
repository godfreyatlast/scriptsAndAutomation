...
if [ "$?" = "0" ]; then
	...
	STATUS="ALERT";
	cat $LOGFILE | mailx -s "cmdb_daily_changed_clients_updater.sh - $STATUS" $EMAIL;
fi
...
#pull csv
wget --no-check-certificate --user=administrator --password=administrator https://cvglpdpa01.td.afg:9002/dpa-api/scheduledreport/results/cmdb_daily_changed_clients_updater.csv
error_exit_func
...
#send to isilon
smbclient -U ga/cvgavmvic%0Bj5CIwlgdYa //cvgisln01.nas.afg/sdrive --directory Shared/Oncall/CMDB -c 'put "cmdb_daily_changed_clients_updater.csv"'
error_exit_func

#remove file
rm -f ./cmdb_daily_changed_clients_updater.sh*
error_exit_func
